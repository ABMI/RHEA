...
shinyApp(
    ...
    tabPanel("Cohort level",
             tabBox(title = tags$b(paste("Cohort (n =",n_distinct(Cohort$SUBJECT_ID), ")")),
                    ...
                    tabPanel(title = tags$b("TREATMENT"),
                             fluidRow(
                               div(style = "display: flex; align-items: center;",
                                   h4(strong("Cancer stage"), style = "margin-right: 20px;"),  # Add the title here
                                   selectInput(inputId = "ca_stage",
                                              label = "",
                                              choices = c("1", "2", "3", "4")
                                              )
                                   )
  ...
  , server <- function(input, output, session) {
    ...
    output$seletedTrend <- renderUI({
      ...
      totalPatients <- nrow(Deduplication %>% distinct(SUBJECT_ID))
      ...
      if(input$Trend == "General"){
        ...
        deathUI <- fluidRow(helpText(paste0("Total number of deaths: ", totalPatients)),
                            column(6,
    ...
    output$Sankey <- renderUI({
      ...
      csv_files <- list.files(path = paste0(outputFolder, "/stage", ca_stage), pattern = "\\.csv$", full.names = TRUE)
      ...
      geneUI <- switch(input$chooseGene,
                       "Microsatellite instability(MSI)" = list(
                         ...
                         column(9,
                                ...
                                fluidRow(box(
                                  title = tags$b("Treatment Pathway"),
                                  width = 12,
                                  uiOutput("MSIpathway")))
                         )
                       ),
    ...
    # Gene Pathway
    output$MSIpathway <- renderUI({
      selected_level <- input$MSI_select
      level <- substring(selected_level, 1, 1)

      csv_files <- list.files(path = paste0(outputFolder, "/GeneMutation"), pattern = "\\.csv$", full.names = TRUE)
      Pathway_file <- csv_files[grep(paste0("MSI_", level), csv_files)]
      link <- read.csv(Pathway_file[1])
      node <- read.csv(Pathway_file[2])
      treatment <- list(nodes=node, links=link)
      treatmentPathway <- networkD3::sankeyNetwork(Links = treatment$links,
    ...
    output$KRASpathway <- renderUI({
      selected_level <- input$KRAS_select
      level <- substring(selected_level, 1, 1)

      csv_files <- list.files(path = paste0(outputFolder, "/GeneMutation"), pattern = "\\.csv$", full.names = TRUE)
      Pathway_file <- csv_files[grep(paste0("KRAS_", level), csv_files)]
      link <- read.csv(Pathway_file[1])
      node <- read.csv(Pathway_file[2])
      treatment <- list(nodes=node, links=link)
      treatmentPathway <- networkD3::sankeyNetwork(Links = treatment$links,
    ...
    output$NRASpathway <- renderUI({
      selected_level <- input$NRAS_select
      level <- substring(selected_level, 1, 1)

      csv_files <- list.files(path = paste0(outputFolder, "/GeneMutation"), pattern = "\\.csv$", full.names = TRUE)
      Pathway_file <- csv_files[grep(paste0("NRAS_", level), csv_files)]
      link <- read.csv(Pathway_file[1])
      node <- read.csv(Pathway_file[2])
      treatment <- list(nodes=node, links=link)
      treatmentPathway <- networkD3::sankeyNetwork(Links = treatment$links,
    ...
    output$BRAFpathway <- renderUI({
      selected_level <- input$BRAF_select
      level <- substring(selected_level, 1, 1)

      csv_files <- list.files(path = paste0(outputFolder, "/GeneMutation"), pattern = "\\.csv$", full.names = TRUE)
      Pathway_file <- csv_files[grep(paste0("BRAF_", level), csv_files)]
      link <- read.csv(Pathway_file[1])
      node <- read.csv(Pathway_file[2])
      treatment <- list(nodes=node, links=link)
      treatmentPathway <- networkD3::sankeyNetwork(Links = treatment$links,
