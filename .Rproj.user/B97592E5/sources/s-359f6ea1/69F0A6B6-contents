...
#' @export heatmapData
heatmapData<-function(connection,
                      cohortDatabaseSchema,
                      standardCycle,
                      targetCohortIds,
                      outputFolder){

  targetCohortIds <- targetCohortIds[targetCohortIds %in% unique(standardCycle$cohortDefinitionId)]
  heatmapPlotData <-data.table::rbindlist(
    lapply(targetCohortIds, function(targetId){
      plotData <- distributionTable(standardData = standardCycle,
                                    targetId = targetId)
