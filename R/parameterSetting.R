...
listForm <- function(regimenInstance,
  ...
  tryCatch({filteredRegimenLists <- regimenInstance[unlist(lapply(regimenInstance,
                                                                  `[`,
                                                                  "conceptId")) %in% targetRegimenConceptId][[1]]
...
parameterSetting <- function(targetRegimenConceptIds){
  ...
  pathToRjson <- system.file("Json", jsonFile, package = "RHEA")
